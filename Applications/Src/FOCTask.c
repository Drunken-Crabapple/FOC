...
static void FOCTask_InitObjects(void) {
    ...
    PID_t pid_speed;
    PID_t pid_angle;
    PID_Init(&pid_current_q, PID_DELTA_TYPE, FOC_CURRENT_KP, FOC_CURRENT_KI, FOC_CURRENT_KD,
             NAN, NAN, 1.0f, -1.0f);
    PID_Init(&pid_current_d, PID_DELTA_TYPE, FOC_CURRENT_KP, FOC_CURRENT_KI, FOC_CURRENT_KD,
             NAN, NAN, 1.0f, -1.0f);
    PID_Init(&pid_speed, PID_POSITION_TYPE, FOC_SPEED_KP, FOC_SPEED_KI, FOC_SPEED_KD,
             1e4f, -1e4f, FOC_MAX_CURRENT, -FOC_MAX_CURRENT);
    PID_Init(&pid_angle, PID_POSITION_TYPE, FOC_ANGLE_KP, FOC_ANGLE_KI, FOC_ANGLE_KD,
             NAN, NAN, FOC_MAX_SPEED, -FOC_MAX_SPEED);

    QD4310_InitObject(&qd4310, FOC_POLE_PAIRS, 5000U, 20000U,
                      &current_q_filter.base, &current_d_filter.base, &speed_filter.base,
