...
void Communicate_SendTelemetry(RxCommand::PlugType plug);
...
void StartCommunicateTask(void *argument) {
    ...
    TickType_t last_uart_tx_tick = xTaskGetTickCount();
    const TickType_t uart_tx_period_tick = pdMS_TO_TICKS(20);
    while (true) {
        if (xQueueReceive(xQueue1, &rx_command, uart_tx_period_tick) == pdPASS) {
            ...
            switch (rx_command.cmd.fields.cmd_type) {
                ...
                case RxCommand::CmdType::StepAngleCtrl: // 角度步进
                    ...
                    break;
                case RxCommand::CmdType::Calibrate: // 校准
                    qd4310.calibrate();
                    break;
            ...
            if (RxCommand::is_CmdType(rx_command.cmd.fields.cmd_type)) {
                Communicate_SendTelemetry(rx_command.plug);
            }
        }

        if ((xTaskGetTickCount() - last_uart_tx_tick) >= uart_tx_period_tick) {
            Communicate_SendTelemetry(RxCommand::PlugType::UART);
            last_uart_tx_tick = xTaskGetTickCount();
        }
    }
}

void Communicate_SendTelemetry(const RxCommand::PlugType plug) {
    static char uart_tx_buffer[96]{};

    static union CanTxData {
        struct __attribute__((packed)) {
            uint8_t id;          // 电机ID
    ...
    if (plug == RxCommand::PlugType::CAN) {
        can_tx_data.data.id = qd4310.ID;
        can_tx_data.data.motor_state = qd4310.started ? 0x01 : 0x00;
        can_tx_data.data.error_code = 0x00;
        can_tx_data.data.current = qd4310.getCurrent() / 10 * INT16_MAX;
        can_tx_data.data.speed = qd4310.getSpeed() / 1000 * INT16_MAX;
        can_tx_data.data.angle = qd4310.getAngle() / (2 * numbers::pi_v<float>) * UINT16_MAX;
        can_tx_data.data.crc8 = CRC8(can_tx_data.raw, sizeof(can_tx_data.raw) - 1, 0x07, 0x00, 0x00, false, false);
        CAN_Transmit(sizeof(can_tx_data.raw) - 2, can_tx_data.raw + 1);
    } else if (plug == RxCommand::PlugType::UART) {
        if (huart3.gState != HAL_UART_STATE_READY) {
            return;
        }

        const int tx_len = std::snprintf(uart_tx_buffer,
                                         sizeof(uart_tx_buffer),
                                         "%.3f,%.3f,%.6f,%.3f,%.3f,%.2f,%.6f\r\n",
                                         static_cast<double>(qd4310.getTargetCurrent()),
                                         static_cast<double>(qd4310.getCurrent()),
                                         static_cast<double>(qd4310.getDCurrent()),
                                         static_cast<double>(qd4310.getQVoltage()),
                                         static_cast<double>(qd4310.getTargetSpeed()),
                                         static_cast<double>(qd4310.getSpeed()),
                                         static_cast<double>(qd4310.getAngle()));

        if (tx_len > 0) {
            const auto length = static_cast<uint16_t>(
                tx_len < static_cast<int>(sizeof(uart_tx_buffer)) ? tx_len : sizeof(uart_tx_buffer) - 1);
            HAL_UART_Transmit_DMA(&huart3, reinterpret_cast<uint8_t *>(uart_tx_buffer), length);
        }
    }
}
