...
void QD4310_InitObject(QD4310_t *motor,
    ...
    FOC_InitObject(&motor->foc, pole_pairs, ctrl_frequency, current_ctrl_frequency,
...
void QD4310_Init(QD4310_t *motor) {
    FOC_Init(&motor->foc);
    if (!motor->storage->initialized) motor->storage->init(motor->storage);
...
void QD4310_RestoreCalibration(QD4310_t *motor) {
    QD4310_SetPID(motor, FOC_SPEED_KP, FOC_SPEED_KI, FOC_SPEED_KD,
    ...
    QD4310_FreezeStorageCalibration(motor,
...
void QD4310_LoadStorageCalibration(QD4310_t *motor) {
    ...
    if ((storage_status & QD4310_STORAGE_ANTICOGGING_CALIBRATE_OK) == QD4310_STORAGE_ANTICOGGING_CALIBRATE_OK) {
        motor->storage->read(motor->storage, 0x800U, motor->foc.anticogging_map, sizeof(motor->foc.anticogging_map));
...
void QD4310_FreezeStorageCalibration(QD4310_t *motor, QD4310_StorageStatus storage_type) {
    ...
    if ((storage_type & QD4310_STORAGE_ANTICOGGING_CALIBRATE_OK) == QD4310_STORAGE_ANTICOGGING_CALIBRATE_OK) {
        motor->storage->write(motor->storage, 0x800U, motor->foc.anticogging_map, sizeof(motor->foc.anticogging_map));
    }
