...
static inline void LowPassFilter2_Init(LowPassFilter2_t *filter,
                                       float ts,
                                       float fc,
                                       float damping_ratio)
{
    memset(filter, 0, sizeof(*filter)); //把结构体里面参数全部清零
    ...
    filter->base.apply = LowPassFilter2_Apply;      //绑定函数 filter里面的base结构体里面的指针函数就是指向这里
