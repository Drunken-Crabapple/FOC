...
void FOC::loopCtrl() {
    ...
    /**3.计算转速**/
    const float speed_delta = wrap(Angle - PreviousAngle, -numbers::pi_v<float>, numbers::pi_v<float>);
    Speed = SpeedFilter(speed_delta * 60 * CurrentCtrlFrequency * numbers::inv_pi_v<float> * 0.5f);
    PreviousAngle = Angle;
